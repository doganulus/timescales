...
hyperfine \
    --warmup 1 \
    --runs 2 \
    --export-json "/timescales/results/dejavu.discrete.djv.1M.$(date +%s).json" \
    --command-name Access \
    ...
    --command-name TimedCommands \
        "dejavu /examples/timed/commands/spec.timed.txt /examples/timed/commands/log.1000k.timed.csv > /dev/null 2>&1" 
