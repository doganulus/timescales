...
class bounded_universality_after_q:
	...
	def generate_trace(upper_bound, duration=1000, failing_end=True):
		...
		while time < duration:
			...
			rows += [TimedData(time=time, data=Data(q=1,p=1))]
			time += 1
...
class bounded_universality_before_r:
	...
	def generate_trace(upper_bound, duration=1000, failing_end=True):
		...
		while time < duration:
			...
			rows += [TimedData(time=time, data=Data(p=1,r=1))]
			time += 1
...
class bounded_universality_between_q_and_r:
	...
	def generate_trace(lower_bound, upper_bound, duration=1000, failing_end=True):
		...
		while time < duration:
			...
			rows += [TimedData(time=time, data=Data(q=1,p=1,r=0))]
			time += 1
			...
			rows += [TimedData(time=time, data=Data(q=0,p=1,r=1))]
			time += 1
